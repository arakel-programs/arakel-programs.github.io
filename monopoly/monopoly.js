function Game() {
	...
	this.auction = function() {
		...
		popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Auction <span id='propertyname'></span></div><div>Highest Bid = $<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, it is your turn to bid.</div<div><input id='bid' title='Enter an amount to bid on " + s.name + ".' style='width: 291px;' /></div><div class='bid-btns'><input type='button' value='Bid' onclick='game.auctionBid();' title='Place your bid.' /><input type='button' value='Pass' title='Skip bidding this time.' onclick='game.auctionPass();' /><input type='button' value='Exit Auction' title='Stop bidding on " + s.name + " altogether.' onclick='if (confirm(\"Are you sure you want to stop bidding on this property altogether?\")) game.auctionExit();' /></div>", "blank");
...
function updateMoney() {
    ...
    var quicksStats = document.getElementById("quickstats");
    
	quicksStats.style.borderColor = "white";
	quicksStats.style.background = p.color;
	quicksStats.style.color = "white";
	quicksStats.style.boxShadow = "0 0 5px " + p.color;
...
window.onload = function() {
	...
	$("#buy-menu-item").click(function() {
		...
		$("#manage").hide();
		$("#trade").hide();
        $("#buy-menu-item").addClass("select");
        $("#manage-menu-item").removeClass("select");
        $("#trade-menu-item").removeClass("select");
	...
	$("#manage-menu-item").click(function() {
		...
		$("#buy").hide();
		$("#trade").hide();
        $("#buy-menu-item").removeClass("select");
        $("#manage-menu-item").addClass("select");
        $("#trade-menu-item").removeClass("select");
	});
	...
	$("#trade-menu-item").click(function() {
        ...
        $("#buy-menu-item").removeClass("select");
        $("#manage-menu-item").removeClass("select");
        $("#trade-menu-item").addClass("select");
	});
